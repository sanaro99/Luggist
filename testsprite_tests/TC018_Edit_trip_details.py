...
async def run_test():
    ...
    try:
        ...
        # -> Fill the New trip form fields (name, destination, start, end) and submit the form to create the trip.
        ...
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("2026-06-25")
        
        # -> Click the trip options (kebab) button (interactive element index 367) to open the trip menu so 'Edit trip' can be selected.
        # button aria-label="Trip options"
        elem = page.locator("xpath=/html/body/main/div/div/div/div[2]/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'Edit trip' menu item (interactive element index 429) to open the edit trip form.
        # button "Edit trip"
        elem = page.locator("xpath=/html/body/main/div/div/div/div[2]/div[2]/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Input the new trip name and destination into the Edit trip modal and click 'Save changes' to persist updates.
        # text input placeholder="e.g. Japan, two weeks"
        elem = page.locator("xpath=/html/body/div[3]/div[2]/div[2]/form/div/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("Test Trip Edited")
        
        # -> Input the new trip name and destination into the Edit trip modal and click 'Save changes' to persist updates.
        # text input placeholder="e.g. Tokyo"
        elem = page.locator("xpath=/html/body/div[3]/div[2]/div[2]/form/div[2]/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("Barcelona")
