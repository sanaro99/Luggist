...
async def run_test():
    ...
    try:
        ...
        # -> Click the '＋ Create a trip' button (element index 6) to open the trip creation form/modal.
        # button "＋ Create a trip"
        elem = page.locator("xpath=/html/body/main/div/div[2]/div/div/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill the trip name and enter future start/end dates (YYYY-MM-DD) so the Create button becomes enabled, then wait for the UI to update.
        # text input placeholder="e.g. Japan, two weeks"
        elem = page.locator("xpath=/html/body/div[3]/div[2]/div[2]/form/div[2]/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("Future Test Trip")
        ...
        # -> Click the 'Create trip' button (element index 165) to submit the form and create the trip.
        # button "Create trip"
        elem = page.locator("xpath=/html/body/div[3]/div[2]/div[2]/form/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the '← All trips' link (element 385) to return to the home dashboard so the dashboard stats and upcoming trip info can be verified.
        # link "← All trips"
